...
public class JugadorService implements IJugadorService {
	...
	public void eliminarTiradesJugador(String idJugador) throws Exception {
		...
		if (jugadorOptional.isPresent()) {
			Jugador jugador = jugadorOptional.get();
			if (tiradaRepository.countByJugador(jugador) > 0) {
	...
	public Iterable<JugadorDTO> llistarJugadors() throws Exception {
		...
		for(Jugador jugador:jugadors) {
			...
			if (numeroTotalDeTirades > 0) {
				Float numeroDeTiradesGuanyades = countAllByJugadorWithValorDausIgualASet(jugador);
	...
	public Float obtenirPercentatgeMigExitTotalJugadors() throws Exception {
		...
		Float numeroTotalDeTirades = (float) tiradaRepository.count();
		
		if (numeroTotalDeTirades > 0) {
			Float numeroDeTiradesGuanyades = countAllWithValorDausIgualASet();
			Float percentatgeMigEncerts = (numeroDeTiradesGuanyades * 100) / numeroTotalDeTirades;
	...
	private Float countAllByJugadorWithValorDausIgualASet(Jugador jugador) {
		
		Float valorDausIgualASet = (float) 0;
		Iterable<Tirada> tirades = tiradaRepository.findAllByJugador(jugador);
		for(Tirada tirada:tirades) {
			if (tirada.getValorDau1() + tirada.getValorDau2() == 7) {
				valorDausIgualASet = valorDausIgualASet + 1;
			}
		}
		return valorDausIgualASet;
	}
	
	private Float countAllWithValorDausIgualASet() {
		
		Float valorDausIgualASet = (float) 0;
		Iterable<Tirada> tirades = tiradaRepository.findAll();
		for(Tirada tirada:tirades) {
			if (tirada.getValorDau1() + tirada.getValorDau2() == 7) {
				valorDausIgualASet = valorDausIgualASet + 1;
			}
